...
public class BaseTest {
	...
	public void beforeMethod() {
		utils.log().info("Super Beefore method");
	...
	public synchronized void afterMethod(ITestResult result) throws Exception {
		...
		if (result.getStatus() == 2) {
			...
			try {
				...
				stream.close();
				utils.log("video path: " + videoDir + File.separator + result.getName() + ".mp4");
			} catch (Exception e) {
				utils.log("error during video capture" + e.toString());
			} finally {
				if (stream != null) {
					stream.close();
				}
...
//		server = getAppiumServerDefault(); // -> If using Windows, uncomment this statement and comment above statement
	...
	public AppiumDriverLocalService getAppiumService() {
		...
		environment.put("PATH", "Library/Java/JavaVirtualMachines/jdk-15.0.2.jdk/Contents/Home/bin:/Users/vishnu/Library/Android/sdk/tools:/Users/vishnu/Library/Android/sdk/platform-tools:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/Library/Apple/usr/bin" + System.getenv("PATH"));
		environment.put("ANDROID_HOME", "/Users/vishnu/Library/Android/sdk");
...
//				.withArgument(() -> "--allow-insecure","chromedriver_autodownload")
	...
	@BeforeTest
	  public void beforeTest(@Optional("androidOnly")String emulator, String platformName, String udid, String deviceName, 
		...
		try {
			...
			String xmlFileNmae = "strings/strings.xml";

			inputStream = getClass().getClassLoader().getResourceAsStream(propFileName);
			props.load(inputStream);
			setProps(props);

			stringis = getClass().getClassLoader().getResourceAsStream(xmlFileNmae);
			...
			e.printStackTrace();
			utils.log().error("Something went wrong while initializing Appium Driver");
			throw e;
		} finally {
			if (inputStream != null) {
				inputStream.close();
	...
	public void waitForVisibility(MobileElement e) {
		WebDriverWait wait = new WebDriverWait(getDriver(), TestUtils.WAITs);
...
//		  scrollObject.put("toVisible", "sdfnjksdnfkld");
	...
	public void afterTest() {
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			utils.log().error("Something went wrong while quiting Appium Driver");
		}
